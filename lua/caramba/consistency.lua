...
M._learn_naming_conventions = function(root)
  ...
  if not intelligence.db.symbols then
    vim.notify("Please run :CarambaIndexProject first", vim.log.levels.WARN)
    return
  end
...
M._analyze_import_patterns = function(root)
  ...
  vim.fn.jobstart({"sh", "-c", "find " .. root .. " -name '*.js' -o -name '*.ts' -o -name '*.py' | head -20"}, {
    stdout_buffered = true,
    on_stdout = function(_, data, _)
      if type(data) == 'table' then for _, l in ipairs(data) do if l and l ~= '' then table.insert(files, l) end end end
    end,
...
M._detect_layer_violations = function(root)
  ...
  for _, layer in ipairs(layers) do
    local layer_files = {}
    local co2 = coroutine.running(); local done2 = false
    vim.fn.jobstart({"sh", "-c", "find " .. root .. " -path '*/" .. layer.name .. "/*' -name '*.js' -o -name '*.ts' 2>/dev/null"}, {
      stdout_buffered = true,
      on_stdout = function(_, data, _)
        if type(data) == 'table' then for _, l in ipairs(data) do if l and l ~= '' then table.insert(layer_files, l) end end end
      end,
      on_exit = function() done2 = true; if co2 then coroutine.resume(co2) end end,
    })
    if co2 then coroutine.yield() end
...
M._learn_style_patterns = function(root)
  -- Sample some files
  local files = {}
  local co3 = coroutine.running(); local done3 = false
  vim.fn.jobstart({"sh", "-c", "find " .. root .. " -name '*.js' -o -name '*.ts' -o -name '*.py' | head -10"}, {
    stdout_buffered = true,
    on_stdout = function(_, data, _)
      if type(data) == 'table' then for _, l in ipairs(data) do if l and l ~= '' then table.insert(files, l) end end end
    end,
    on_exit = function() done3 = true; if co3 then coroutine.resume(co3) end end,
  })
  if co3 then coroutine.yield() end
...
M.check_file = function(opts)
  ...
  if not M.patterns or not M.patterns.style or not M.patterns.naming then
    vim.notify("No consistency patterns found. Run :CarambaLearnPatterns first to analyze your project.", vim.log.levels.WARN)
    return {}
  end
