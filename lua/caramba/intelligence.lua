...
M.analyze_type_flow = function(symbol_name)
  ...
  do
    ...
    vim.fn.jobstart({"sh", "-c", usage_cmd}, {
      stdout_buffered = true,
      on_stdout = function(_, data, _)
        if type(data) == 'table' then
          for _, l in ipairs(data) do if l and l ~= '' then table.insert(usages, l) end end
        end
      end,
...
M._find_direct_dependents = function(symbol_name)
  ...
  do
    ...
    vim.fn.jobstart({"sh", "-c", cmd}, {
      ...
      on_stdout = function(_, data, _)
        if type(data) == 'table' then
          for _, l in ipairs(data) do if l and l ~= '' then table.insert(files, l) end end
        end
      end,
      on_exit = function() done = true; if co then coroutine.resume(co) end end,
    })
      ...
      if not loc.file:match("test") and not loc.file:match("spec") then
        ...
        do
          ...
          vim.fn.jobstart({"sh", "-c", usage_cmd}, {
            stdout_buffered = true,
            on_stdout = function(_, data, _)
              if type(data) == 'table' then usage_output = table.concat(data, "\n") end
            end,
            on_exit = function() done = true; if co then coroutine.resume(co) end end,
          })
          if co then coroutine.yield() end
  ...
  if #symbol_list > 0 then
    ...
    do
      ...
      vim.fn.jobstart({"sh", "-c", cmd}, {
        stdout_buffered = true,
        on_stdout = function(_, data, _)
          if type(data) == 'table' then files_with_symbols = table.concat(data, "\n") end
        end,
        on_exit = function() done = true; if co then coroutine.resume(co) end end,
      })
      if co then coroutine.yield() end
  ...
  for symbol_name, _ in pairs(M.db.symbols) do
    ...
    do
      ...
      vim.fn.jobstart({"sh", "-c", cmd}, {
        stdout_buffered = true,
        on_stdout = function(_, data, _)
          if type(data) == 'table' then output = table.concat(data, "\n") end
        end,
        on_exit = function() done = true; if co then coroutine.resume(co) end end,
      })
      if co then coroutine.yield() end
