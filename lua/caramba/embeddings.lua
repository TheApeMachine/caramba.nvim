...
M.generate_embedding_openai = function(text, callback)
  ...
  local request_id = tostring(os.time()) .. "_embed_" .. math.random(10000)
  -- Log request body redacted (no key)
  local ok_body, body_tbl = pcall(vim.json.decode, body_json)
  logger.info("OpenAI embeddings request", {
    id = request_id,
    url = "https://api.openai.com/v1/embeddings",
    headers = { ["Content-Type"] = "application/json", ["Authorization"] = "Bearer ***" },
    body = ok_body and body_tbl or body_json,
  })
  local job = Job:new({
    command = "curl",
    args = {
    ...
    on_exit = function(j, return_val)
      if return_val ~= 0 then
        logger.error("OpenAI embeddings exit", { id = request_id, code = return_val })
        callback(nil, "Request failed")
        return
      end

      local response = table.concat(j:result(), "\n")
      logger.info("OpenAI embeddings response", { id = request_id, raw_len = #response, raw_preview = response:sub(1, 4000) })
      local ok, data = pcall(vim.json.decode, response)

      if ok and data.data and data.data[1] and data.data[1].embedding then
        callback(data.data[1].embedding, nil)
      else
        logger.error("OpenAI embeddings parse_error", { id = request_id, response_preview = response:sub(1, 2000) })
